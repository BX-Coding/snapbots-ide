...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log("Processing soccer mode request");
    ...
    const modalEndpoint = process.env.SNAPBOT_SOCCER_MODAL_ENDPOINT;
    
    if (!modalEndpoint) {
      console.error("Missing SNAPBOT_SOCCER_MODAL_ENDPOINT environment variable");
      return res.status(500).json({ 
        error: 'Soccer endpoint not configured',
        details: 'Missing SNAPBOT_SOCCER_MODAL_ENDPOINT environment variable'
      });
    }

    console.log("Using soccer endpoint:", modalEndpoint);
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        console.error('Request to Soccer Modal server timed out after 60 seconds');
    ...
    console.error('Error proxying to Soccer Modal server:', error);
    return res.status(500).json({ error: 'Failed to proxy request to Soccer Modal server', details: error.message });
  }
} 